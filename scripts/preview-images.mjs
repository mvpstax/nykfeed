...
async function articleHtml(url, allowedHosts, headers, timeout) {
  ...
  try {
    ...
    for (let hop = 0; hop < 5; hop++) {
      if (current.protocol !== "https:" || !allowedHosts.has(current.hostname) || current.username || current.password || current.port) {
        throw new Error("Unapproved article host");
      }
...
export async function enrichImages(items, previous, config, headers) {
  ...
  const allowed = new Set(config.sources.flatMap(source => [new URL(source.url).hostname, ...(source.image_hosts || [])]));
  const pending = [];
  ...
  for (const item of items) {
    ...
    if (item.image_url) {
      ...
      item.image_source = "source_feed";
      continue;
    }
    if (item.type !== "article") {
      item.image_status = "not_provided";
      continue;
    }
  ...
  return {
    available: items.filter(item => item.image_url).length,
    missing: items.filter(item => !item.image_url).length,
    article_pages_checked: pending.length
  };
}
